            ...
            @foreach ($data as $key => $data)
                @if (count($data) > 0)
                    @if ($key == 'pemberian_obat')
                        <div class="block p-8 col-span-2 lg:col-auto bg-white rounded-md h-auto">
                            <p class="text-2xl font-bold mb-5">{{ ucwords(str_replace('_', ' ', $key)) }}</p>
                            <div class="overflow-y-auto max-h-72 mb-5">
                                <ul class="list-none">
                                    @foreach ($data as $row)
                            ...
                            <div class="flex place-content-end p-2">
                                <a href="{{ route('rekmed.detail_medis_data', ['id' => $penghuni->id, 'data' => $key]) }}">
                                    <button class="p-3 rounded-md bg-indigo-200 hover:bg-indigo-600 hover:text-gray-50">
            ...
            @if ($data->count() > 0)
                <div class="block p-8 bg-white col-span-2 mt-4 rounded-md">
                    <div class="grid grid-cols-2 gap-4 justify-between">
                        <div class="block col-span-2 border-b pb-3 mb-5">
                            <p class="text-2xl font-bold">{{ ucwords(str_replace('_', ' ', $key)) }}</p>
                        </div>
                        @foreach (['sistole', 'diastole'] as $k)
                            <div class="block col-span-2 2xl:col-auto h-auto">
                    ...
                    <div class="grid grid-cols-3 gap-4 justify-between">
                        ...
                        @foreach (['pagi', 'siang', 'sore'] as $k)
                            @if (collect($data)->whereNotNull($k)->count() > 0)
                                <div class="block col-span-3 2xl:col-auto h-auto">
                                    <p class="text-2xl font-bold">{{ ucwords(str_replace('_', ' ', $k)) }}</p>
