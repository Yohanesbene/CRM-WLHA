        ...
        <div class="flex-auto bg-indigo-50 py-6 px-10">
            <div class="block p-8 bg-white rounded-md">
                <header class="flex items-center justify-center mb-12">
                    <h2 class="font-semibold uppercase text-2xl">Ubah Penghuni</h2>
                </header>
                @if (count($errors) > 0)
                ...
                @endif
                <form method="POST" action="{{ route('penghuni.prosesubah') }}" enctype="multipart/form-data">
                    @csrf
                    <!-- ID -->
                    <x-input type="number" name="id" value="{{ $penghuni['id'] }}" hidden></x-input>
                    <x-input type="text" name="no_induk" value="{{ $penghuni['no_induk'] }}" hidden></x-input>
                    <x-input type="text" name="old_foto" value="{{ $penghuni['foto'] }}" hidden></x-input>

                    <!-- nama Input -->
                    <x-label for="editnama" :value="__('Nama Penghuni')" />
                    <x-input invalid="{{ $errors->has('nama') ? 'true' : 'false' }}" id="editnama" type="text" name="nama" value="{{ old('nama') ? old('nama') : $penghuni['nama'] }}" placeholder=" Masukkan Nama Penghuni Baru" autocomplete="off" />
                    ...
                    <x-input invalid="{{ $errors->has('tgl_lahir') ? 'true' : 'false' }}" id="edittgl_lahir" type="date" name="tgl_lahir" value="{{ old('tgl_lahir') ? old('tgl_lahir') : $penghuni['tgl_lahir'] }}" autocomplete="off" />
                    ...
                    @foreach (['pria', 'wanita'] as $gender)
                        <x-label invalid="{{ $errors->has('gender') ? 'true' : 'false' }}" for="edit{{ $gender }}" class="flex p-2 cursor-pointer">
                            <input class="my-auto transform scale-125" type="radio" id="edit{{ $gender }}" name="gender" value="{{ $gender }}" @if (old('gender') ? old('gender') : $penghuni['gender'] == $gender) checked="checked" @endif />
                    ...
                    <div class="flex flex-wrap">
                        @foreach (['katolik', 'kristen', 'islam', 'hindu', 'budha', 'kepercayaan'] as $agama)
                            <x-label invalid="{{ $errors->has('agama') ? 'true' : 'false' }}" for="edit{{ $agama }}" class="flex p-2 cursor-pointer">
                                <input class="my-auto transform scale-125" type="radio" id="edit{{ $agama }}" name="agama" value="{{ $agama }}" @if (old('agama') ? old('agama') : $penghuni['agama'] == $agama) checked="checked" @endif />
                    ...
                    <x-input invalid="{{ $errors->has('asal_daerah') ? 'true' : 'false' }}" id="asal_daerah" type="text" name="asal_daerah" value="{{ old('asal_daerah') ? old('asal_daerah') : $penghuni['asal_daerah'] }}"
                    ...
                    <x-input invalid="{{ $errors->has('penanggung_jawab') ? 'true' : 'false' }}" id="editPenanggungJawab" type="text" name="penanggung_jawab" value="{{ old('penanggung_jawab') ? old('penanggung_jawab') : $penghuni['penanggung_jawab'] }}" placeholder=" Masukkan Penanggung Jawab Penghuni Baru" autocomplete="off" />
                    ...
                    <x-input invalid="{{ $errors->has('alamat') ? 'true' : 'false' }}" id="editalamat" type="text" name="alamat" value="{{ old('alamat') ? old('alamat') : $penghuni['alamat'] }}" placeholder="Masukkan Alamat Penghuni" autocomplete="off" />
                    ...
                    <x-input invalid="{{ $errors->has('notelp') ? 'true' : 'false' }}" id="editnotelp" type="number" name="notelp" value="{{ old('notelp') ? old('notelp') : $penghuni['notelp'] }}" placeholder="Masukkan No Telepon Penghuni" autocomplete="off" />
                    ...
                    <x-input invalid="{{ $errors->has('kontak_darurat') ? 'true' : 'false' }}" id="editDarurat" type="text" name="kontak_darurat" value="{{ old('kontak_darurat') ? old('kontak_darurat') : $penghuni['kontak_darurat'] }}" placeholder=" Masukkan Kontak Darurat Penghuni Baru" autocomplete="off" />
                    ...
                    <x-input invalid="{{ $errors->has('notelp_darurat') ? 'true' : 'false' }}" id="editnotelp_darurat" type="number" name="notelp_darurat" value="{{ old('notelp_darurat') ? old('notelp_darurat') : $penghuni['notelp_darurat'] }}" placeholder="Masukkan No Telepon Penghuni" autocomplete="off" />
                    ...
                    <x-input invalid="{{ $errors->has('ruang') ? 'true' : 'false' }}" id="editruang" type="text" name="ruang" value="{{ old('ruang') ? old('ruang') : $penghuni['ruang'] }}" placeholder="Masukkan Ruang Rawat" autocomplete="off" />
                    ...
                    <div class="flex space-x-4 items-center">
                        <img id="edittambahPreviewImg" src="{{ $penghuni['foto'] != null? URL::to('/photos/' . $penghuni['foto']): 'https://user-images.githubusercontent.com/507615/54591670-ac0a0180-4a65-11e9-846c-e55ffce0fe7b.png' }}" class="object-cover w-40 h-40 rounded-full shadow border-2 border-gray-300 border-dashed previewImg' }}" alt="profile image">
    ...
    <script>
        ...
        function displayPhoto(file) {
            ...
            if (file) {
                ...
                //     $('#edittambahPreviewImg').attr("src", reader.result);
                // }
                $('#edittambahPreviewImg').attr("src", file);
                // reader.readAsDataURL(file);
            }
        }

        $(document).load(displayPhoto("<?= $penghuni['foto'] ?>"))
    </script>
</x-app-layout>
